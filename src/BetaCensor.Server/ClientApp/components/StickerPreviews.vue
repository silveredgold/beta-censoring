<template>
    <n-tabs size="large" justify-content="space-evenly" @before-leave="beforeTabChange" @update:value="onTabChange">
        <template #prefix>
            Categories
        </template>
        <template #suffix>
            <n-popover trigger="hover" placement="left">
                    ...
                    <n-thing
                        ...
                        title="Previewing your stickers"
                    ...
                    <template #footer><n-text depth="3">Be patient! Previews can take a bit to load.</n-text></template></n-thing>
                </n-popover>
        </template>
        <n-tab-pane v-for="cat in categories" display-directive="show:lazy" v-if="allStickers" :name="cat">
        <template #tab>
            <n-space>
                {{ cat }}
                <n-badge :value="allStickers[cat].length" :max="99" type="success" />
            </n-space>
        </template>
        <n-space>
...
import { NIcon, useNotification, NPopover, NButton, NSpace, NImage, NImageGroup, NTabs, NTabPane, useLoadingBar, NBadge, NThing, NText } from "naive-ui";
